...
class DonorGui:
    ...
    INPUT_FILE_HELP_TEXT = sg.Text('Input files are the .csv or .xlsx files created from input sources like Stripe ' +
                                   ...
                                   'note that Fidelity and QB are expected to be Excel files, Benevity and ' +
                                   'YourCause are expected to be CSV files.\nStripe can either Excel or CSV.',
                                   text_color='black')
    ...
    def display_popup(self, messages):
        # Put the warning messages into alphabetical order to make it easier to read them.
        messages[1:len(messages)-1] = sorted(messages[1:len(messages)-1])
